...
export async function scrapeAdzuna(params: ScrapeParams): Promise<ScrapeResult> {
  ...
  if (!country) {
    ...
    if (
      /\b(canada|toronto|vancouver|montreal|ottawa|calgary|edmonton|winnipeg|ontario|british columbia|alberta|quebec|kitchener|waterloo|hamilton|london on|gta)\b/.test(
        loc,
      )
    ) {
      country = 'ca';
    } else if (
      /\b(united states|usa|new york|san francisco|los angeles|chicago|seattle|austin|boston)\b/.test(
        loc,
      )
    ) {
      country = 'us';
  ...
  for (let page = 1; page <= 2; page++) {
    ...
    if (page > 1) await humanDelay(300, 700);
    const data = await stealthFetchJson<{ results?: AdzunaJob[] }>(url);
    if (data === null) {
      failedPages++;
    ...
    for (const item of items) {
      ...
      if (item.salary_min && item.salary_max)
        salary = `$${item.salary_min.toLocaleString(undefined, { maximumFractionDigits: 0 })} - $${item.salary_max.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
      else if (item.salary_min)
        salary = `From $${item.salary_min.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
      else if (item.salary_max)
        salary = `Up to $${item.salary_max.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
  ...
  return {
    ...
    jobs,
    ...(failedPages > 0
      ? { error: `${failedPages} of 2 pages failed (${jobs.length} jobs from successful pages)` }
      : {}),
  };
}
