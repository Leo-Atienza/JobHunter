...
// NOTE: As of 2026-04, Workday's CXS API returns 422 for most companies
// (API format changed) and 404 for RBC/Sun Life/CIBC (paths moved).
// These companies are kept for when the API format is reverse-engineered.
// The scraper handles errors gracefully and returns [] for failed companies.
...
export async function scrapeWorkday(params: ScrapeParams): Promise<ScrapeResult> {
  ...
  // All companies in parallel — each hits a different Workday subdomain.
  const results = await Promise.allSettled(
    companies.map(async ([displayName, baseUrl, sitePath]) => {
